...
/*
Fixed hash is attached to node. Presently, no benefit.
Also attached to node the fixed address. Presently used to easily reference a node's address. Used only during remove()
to avoid duplicate calls to getAddress (during finding of node to remove and getting found node's index when checking
if table[index] node is equal to the node being removed).
 */
public class HashTableWithFixedHash {
